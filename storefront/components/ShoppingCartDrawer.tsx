...
export function ShoppingCartDrawer({ items, onRemoveItem, onClearCart }: ShoppingCartDrawerProps) {
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <Button variant="outline" className="relative border-amber-600 text-amber-400 hover:bg-amber-600/20">
            ...
            <>
              <ScrollArea className="flex-1 -mx-6 px-6">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-4 p-3 bg-slate-800 rounded-lg border border-amber-600/20">
                      <img
                        src={item.image}
                        alt={item.name}
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-amber-400 text-sm truncate">{item.name}</h4>
                        <p className="text-slate-400 text-sm">Qty: {item.quantity}</p>
                        <p className="text-amber-300 text-sm">{item.price * item.quantity} gold</p>
                      </div>
                      <Button
                        size="sm"
                        variant="ghost"
